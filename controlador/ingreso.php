...
switch ($_GET["op"]) {
    case 'guardaryeditar':
        ...
        if (empty($idingreso)) {
            ...
            $rspta = $ingreso->insertar($idproveedor, $idusuario, $tipo_comprobante, $serie_comprobante, $num_comprobante,
                    $fecha_hora, $impuesto, $total_compra, $_POST["idarticulo"], $_POST["cantidad"], $_POST["precio_compra"],
                    $_POST["precio_venta"]);
    ...
    //Listar los proveedores en un select
    ...
    case 'listarArticulos':
        ...
        while ($reg = $rspta->fetch_object()) {
            ...
            $data[] = array(
                "0" => '<button class="btn btn-warning" onclick="agregarDetalle(' . $reg->idarticulo . ',\'' . $reg->nombre . '\')"> <i class="fa fa-pencil"></i> </button>',
                "1" => $reg->nombre,
                "2" => $reg->categoria,
                "3" => $reg->codigo,
                "4" => $reg->stock,
